...
export class DungeonState extends Schema {
  ...
  @type({ map: "string" }) inspirationPendingTargets = new MapSchema<string>(); // casterSessionId -> targetSessionId
  @type({ map: "number" }) inspirationExtraActiveUses = new MapSchema<number>(); // targetSessionId -> remaining extra active-card replays
  ...
  private ensureEntranceChecked(room: Room): void {
  ...
  createPlayer(id: string, name: string) {
    ...
    this.players.set(id, new Player(name, lobbyPlayerCount));
  ...
  removePlayer(id: string) {
    ...
    const inspirationTargetsToClear: string[] = [];
  ...
  private crossRandomUncrossedMonsterSquares(
    monster: MonsterCard,
    count: number
  ): Array<{ x: number; y: number }> {
    const safeCount = Math.max(0, Math.floor(count || 0));
    const crossedSquares: Array<{ x: number; y: number }> = [];
  ...
  private getValidInspirationTargetSessionId(casterSessionId: string): string | null {
    if (!this.canUseInspiration()) {
      this.inspirationPendingTargets.delete(casterSessionId);
      return null;
    }
  ...
  private grantInspirationExtraActiveUse(sessionId: string): void {
    const existing = this.inspirationExtraActiveUses.get(sessionId);
    const safeExisting =
      typeof existing === "number" && Number.isFinite(existing) ? Math.max(0, Math.floor(existing)) : 0;
    this.inspirationExtraActiveUses.set(sessionId, safeExisting + 1);
  }

  private consumeInspirationExtraActiveUse(sessionId: string): boolean {
    const existing = this.inspirationExtraActiveUses.get(sessionId);
    const safeExisting =
      typeof existing === "number" && Number.isFinite(existing) ? Math.max(0, Math.floor(existing)) : 0;
    if (safeExisting <= 0) {
      ...
      if (mode === "room_2x2_choose_two_twice") {
        for (let y = 0; y < room.height - 1; y++) {
          for (let x = 0; x < room.width - 1; x++) {
            ...
            const hasRequiredAdjacency = crossableSquares.some((pos) =>
              this.isAdjacentToEntranceOrCrossedSquare(room, pos.x, pos.y)
            );
            if (hasRequiredAdjacency) {
              return true;
            }
  ...
  private isMagicCard(card?: { type?: string } | null): boolean {
    return card?.type === MAGIC_CARD_ID;
  }

  private isInspirationCard(card?: { type?: string } | null): boolean {
    return card?.type === INSPIRATION_CARD_ID;
  }

  private canUseInspiration(): boolean {
    return this.players.size >= 2;
  }

  private deactivateInspirationWhenInsufficientPlayers(): void {
    if (this.canUseInspiration()) {
      return;
    }

    const castersToDeactivate: string[] = [];
  ...
  private removeInspirationFromSoloDecks(): void {
    if (this.canUseInspiration()) {
      return;
    }
  ...
  private getCunningRequiredSelectionCount(requiredSteps: 1 | 2 | 3): number {
  ...
  private getCunningStepLengths(requiredSteps: 1 | 2 | 3): number[] {
    if (requiredSteps === 1) return [1];
    if (requiredSteps === 2) return [1, 2];
    return [1, 2, 3];
  }
  ...
  private validateCunningSelectionsForConfirm(
    sessionId: string,
    selections: Selection[],
    card?: Card | null
    ...
    for (const stepLength of stepLengths) {
      ...
      const sameTarget = stepSelections.every(
        (selection) => this.getSelectionTargetKey(selection) === stepTargetKey
      );
      if (!sameTarget) {
        return { valid: false, error: "Cunning card is in an invalid state. Cancel and replay this card." };
      }
      ...
      if (stepSelections[0].kind === "room") {
        const roomStepSelections = stepSelections as Array<{
          kind: "room";
          roomIndex: number;
          x: number;
          y: number;
        ...
        const hasRequiredAdjacency = roomStepSelections.some((selection) =>
          this.isAdjacentToEntranceOrCrossedSquare(room, selection.x, selection.y)
        );
        if (!hasRequiredAdjacency) {
          return {
            valid: false,
            error:
              "At least one square in each Cunning room step must be adjacent to the entrance or an existing crossed square"
          };
        }
      }
      ...
      for (let i = 1; i < stepSelections.length; i++) {
        ...
        if (
          previous.kind === "room" &&
          previous.roomIndex !== (current as { kind: "room"; roomIndex: number }).roomIndex
        ) {
          return { valid: false, error: "Cunning card is in an invalid state. Cancel and replay this card." };
        }

        if (
          previous.kind === "monster" &&
          previous.monsterId !== (current as { kind: "monster"; monsterId: string }).monsterId
        ) {
          return { valid: false, error: "Cunning card is in an invalid state. Cancel and replay this card." };
        }
      }
  ...
  private resolveCunningRoomStep(
    ...
    const hasRequiredAdjacency = stepSelections.some((selection) =>
      this.isAdjacentToEntranceOrCrossedSquare(room, selection.x, selection.y)
    );
    if (!hasRequiredAdjacency) {
      return {
        success: false,
        error:
          "At least one square in the Cunning step must be adjacent to the entrance or an existing crossed square",
        invalidSquare: true
      };
    }
  ...
  selectSquareForCard(sessionId: string, roomIndex: number, x: number, y: number): {
    ...
    if (selectionMode === "horizontal_pair_twice") {
      ...
      const hasRequiredAdjacency =
        this.isAdjacentToEntranceOrCrossedSquare(room, x, y, pendingRoomSquares) ||
    ...
    if (selectionMode === "room_2x2_choose_two_twice") {
      ...
      const crossableOverlaySquares = overlayCoordinates.filter((pos) => {
        ...
        const alreadySelected = roomSelections.some(
          (selection) =>
            selection.roomIndex === roomIndex && selection.x === pos.x && selection.y === pos.y
        );
      ...
      const hasRequiredAdjacency = selectedForPhase.some((pos) =>
        this.isAdjacentToEntranceOrCrossedSquare(room, pos.x, pos.y)
      );
      if (!hasRequiredAdjacency) {
        return {
      ...
      const phaseSelections: Array<{ kind: "room"; roomIndex: number; x: number; y: number }> =
        selectedForPhase.map((pos) => ({
          kind: "room",
          roomIndex,
          x: pos.x,
          y: pos.y
    ...
    if (selectionMode === "row") {
      ...
      for (let checkX = 0; checkX < room.width; checkX++) {
        ...
        if (rowSquare.checked && !rowSquare.entrance) continue;
    ...
    if (this.isMagicCard(card)) {
      const hasOrthogonalNeighbor = roomSelections.some(
        (selection) =>
          selection.roomIndex === roomIndex &&
  ...
  private areOrthogonallyAdjacent(ax: number, ay: number, bx: number, by: number): boolean {
    return (Math.abs(ax - bx) === 1 && ay === by) || (Math.abs(ay - by) === 1 && ax === bx);
  }

  private isAdjacentToEntranceOrCrossedSquare(
    room: Room,
    x: number,
    y: number,
  ...
  confirmCardAction(sessionId: string, data?: any): { success: boolean; message?: string; error?: string; completed?: boolean } {
    ...
    if (this.cardIsMonsterEach(card)) {
      ...
      for (const monster of eligibleMonsters) {
        ...
        if (
          selectedForMonster.length < requiredMin ||
  ...
  private selectMonsterSquareForCard(
    ...
    if (selectionMode === "centered_monster_3x3") {
      const blastSelections: Array<{ kind: "monster"; monsterId: string; x: number; y: number }> = [];
    ...
    if (selectionMode === "monster_swipe_l") {
      ...
      const toCoord = (offset: { dx: number; dy: number }) => ({ x: x + offset.dx, y: y + offset.dy });

      const requiredCoords = requiredOffsets.map(toCoord);
      const optionalCoords = optionalOffsets.map(toCoord);

      const isCrossable = (coord: { x: number; y: number }): boolean => {
      ...
      const swipeSelections: Array<{ kind: "monster"; monsterId: string; x: number; y: number }> = [];
    ...
    if (this.isMagicCard(card)) {
      const hasOrthogonalNeighbor = currentForMonster.some((selection) =>
        this.areOrthogonallyAdjacent(selection.x, selection.y, x, y)
      );
      if (hasOrthogonalNeighbor) {
        return {
