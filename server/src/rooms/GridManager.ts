...
export class GridManager {
   ...
   */
  private roomHasExitInDirection(room: Room, direction: string): boolean {
   ...
   */
  getAllRoomPositions(): Array<{x: number, y: number, roomIndex: number}> {
    const positions: Array<{x: number, y: number, roomIndex: number}> = [];
   ...
   */
  removeRoomAt(x: number, y: number): boolean {
    const key = `${x},${y}`;
